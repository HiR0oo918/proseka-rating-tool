...
- **MASTER以下**（HARD・EXPERT・MASTER Lv.36以下）: 上位 30 譜面の合計 ÷ 30（未入力枠は 0）
- **APPEND**（APPEND と MASTER 37）: 上位 20 譜面の合計 ÷ 20（未入力枠は 0）
- **総合**: `(MASTER以下×3 + APPEND×2) ÷ 5`

譜面定数・判定重み・単曲レートの境界・ベスト枠数・総合の混ぜ方は全員共通です。ユーザー画面からは変えられません。管理者がリポジトリを直してデプロイします。手順は [`data/RATING.md`](data/RATING.md) です。
