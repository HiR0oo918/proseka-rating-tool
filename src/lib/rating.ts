...
export function overallRating(
  otherAverage: number,
  appendAverage: number,
  otherWeight = DEFAULT_OVERALL_OTHER_WEIGHT,
  appendWeight = DEFAULT_OVERALL_APPEND_WEIGHT,
): number {
  const other = Math.max(0, otherWeight);
  const append = Math.max(0, appendWeight);
  const total = other + append;
  if (total === 0) return 0;
  return (otherAverage * other + appendAverage * append) / total;
}
