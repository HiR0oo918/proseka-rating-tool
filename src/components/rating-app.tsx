...
import { matchesQuery as textMatchesQuery } from "@/lib/search";
...
function matchesQuery(chart: Chart, query: string): boolean {
  return textMatchesQuery(chart.title, chart.pronunciation, query);
}
      ...
      {view === "charts" ? (
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
          <div className="flex-1 space-y-1">
            <Label htmlFor={`search-${pool}`}>曲名</Label>
            <Input
              id={`search-${pool}`}
              value={query}
